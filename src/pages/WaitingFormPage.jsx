  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:8080/api/restaurants/${restaurantId}/reservations`,
        {
          method: "POST",
          headers: {
      ...
      <div className={styles.pageContainer}>
        ...
        <div className={styles.contentArea}>
          <div className={styles.formSection}>
            ...
            {errors.people && (
              <p className={styles.errorMessage}>{errors.people}</p>
            )}
          ...
          <div className={styles.menuSection}>
            ...
            />
            {errors.menus && (
              <p className={`${styles.errorMessage} ${styles.menusError}`}>
                {errors.menus}
              </p>
            )}
          ...
          <div className={styles.waitingInfo}>
            {waitingInfoLoading ? ( // ⭐ 로딩 중일 때 표시
              ...
              <>
                <span className={styles.waitingText}>
                  현재 웨이팅:{" "}
                  <strong className={styles.waitingNumber}>
                    {currentWaitingTeams} 팀
                  </strong>
                </span>
                <span className={styles.waitingText}>
                  {" "}
                  (예상 대기 시간:{" "}
                  <strong className={styles.waitingTime}>
                    {predictedWaitingTime} 분
                  </strong>
                  )
                </span>
              </>
