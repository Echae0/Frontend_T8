    ...
    const fetchData = async () => {
      ...
      try {
        if (user && user.memberId) {
          ...
          setRestaurantName(
            restaurantResponse.data.restaurantName || "알 수 없는 식당"
          );
          ...
          if (reservationData.status === "REQUESTED") {
            ...
            setSpecialRequests(
              reservationData.requestDetail || "요청사항 없음"
            );
    ...
    <div>
      ...
      <div className={styles.container}>
        ...
        <p className={styles.currentOrder}>
          현재 나의 순서:{" "}
          <span className={styles.orderNumber}>{myOrder}번째 팀</span>
        </p>
        <p className={styles.expectedTime}>
          예상 대기 시간:{" "}
          <span className={styles.timeValue}>{expectedWaitTime}분</span>
        </p>
